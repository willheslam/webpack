...
class ConcatenatedModule extends Module {
	...
	source(dependencyTemplates, outputOptions, requestShortener) {
		...
		const recurseScopes = (callback, scope) => {
			callback(scope)
			scope.childScopes.forEach(scope => recurseScopes(callback, scope))	
		}
		...
		modulesWithInfo.forEach(info => {
			...
			let allVariables = []
			recurseScopes(scope => allVariables = allVariables.concat(scope.variables), info.moduleScope)

			info.moduleScope.variables
			.forEach((variable, index) =>
				allVariables
				.filter((otherVariable, otherIndex) =>
					otherVariable.name === variable.name && otherIndex !== index
				)
				.forEach(variableAlias =>
					variable.references = variable.references.concat(variableAlias.references)
				)
			)
